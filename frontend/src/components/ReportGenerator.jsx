...
import { User, Phone, Stethoscope, Calendar, Printer, Download, Activity, MessageCircle, Trash2, Search, Save } from 'lucide-react';
...
export default function ReportGenerator() {
  const [patientDetails, setPatientDetails] = useState({
    name: '', age: '', gender: 'Female', phone: '', doctor: 'DR.SELF', date: new Date().toISOString().split('T')[0],
    note: '', address: ''
  });
  ...
  const saveReportToDB = async () => {
    ...
    // Check if any selected test has 0 parameters (mistake in master)
    ...
    try {
      const finalTests = selectedTests.map(t => ({
        ...
        name: t.name,
        category: t.category,
        parameters: JSON.parse(JSON.stringify(t.parameters)), // Force deep copy of parameters
      ...
      console.log("Saving Report Payload:", payload); // Debug log
  ...
  if (loading) return (
    <div className="flex h-screen items-center justify-center bg-gray-50">
       <div className="flex flex-col items-center gap-4">
         ...
         <p className="text-gray-500 font-bold uppercase tracking-widest text-sm">Initializing Lab Workspace...</p>
       </div>
    </div>
  );
      ...
      <div className="w-full lg:w-[450px] bg-white border-r shadow-2xl flex flex-col z-10 lg:h-full overflow-y-auto lg:overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
          <section className="bg-slate-50 p-4 rounded-xl border border-slate-200">
            <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b pb-2">
              <User size={14}/> Patient Credentials
            </h3>
            <div className="grid grid-cols-2 gap-3">
              <input name="name" placeholder="Full Name" onChange={handlePatientChange} value={patientDetails.name} className="border p-2 rounded-lg text-sm font-bold col-span-2 focus:ring-2 focus:ring-indigo-100 outline-none uppercase" />
              ...
              <select name="gender" onChange={handlePatientChange} value={patientDetails.gender} className="border p-2 rounded-lg text-sm font-bold">
                <option>Male</option>
            ...
            <div className="mt-4">
               <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1 tracking-widest">Clinical Note / Interpretation</label>
               <textarea 
                name="note" 
                value={patientDetails.note} 
                onChange={handlePatientChange}
                placeholder="Add special remarks or clinical interpretation..."
          ...
          <section className="bg-slate-50 p-4 rounded-xl border border-slate-200 min-h-[300px]">
            ...
            >
              <option value="" className="text-slate-400 bg-white py-4">-- SELECT DIAGNOSTIC TEST --</option>
              {testList.map(t => (
                 ...
                 {selectedTests.map((test, tIdx) => (
                    <div key={test._id} className="bg-white border border-slate-300 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                       <div className="bg-slate-100/80 px-4 py-3 flex justify-between items-center border-b border-slate-200">
                         <span className="font-black text-[12px] text-indigo-900 uppercase tracking-tight">{test.name}</span>
                       ...
                       {(!test.parameters || test.parameters.length === 0) ? (
                         <div className="p-4 flex flex-col items-center gap-3">
                            <div className="text-[10px] font-black text-orange-600 bg-orange-50 px-4 py-2 rounded-full uppercase border border-orange-100">Zero Parameters Defined</div>
                            <Link to="/manage" className="text-[9px] font-black text-indigo-600 underline uppercase tracking-widest hover:text-indigo-800 transition-all">Manage Master Template</Link>
                         </div>
                       ) : (
                         <div className="p-3 space-y-2 bg-slate-50/30">
                           {test.parameters.map((param, i) => (
                             <div key={i} className="flex items-center gap-3 p-2 bg-white border border-slate-200 rounded-xl shadow-sm">
                               <label className="text-[11px] font-black text-indigo-900 uppercase min-w-[150px] truncate leading-tight">
                                 {param.paramName}
                               </label>
                               <div className="flex-1">
                                  <input 
                                    ...
                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 outline-none focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 text-sm font-black text-emerald-600 transition-all text-center"
                                    placeholder="Value"
                                  />
                               </div>
                             </div>
        ...
        <div className="p-5 border-t bg-slate-50 flex gap-2 shrink-0 sticky bottom-0 z-20">
          <button onClick={saveReportToDB} className="flex-1 bg-white border-2 border-indigo-200 text-indigo-600 py-3 rounded-xl font-black text-sm hover:bg-indigo-50 transition-all flex items-center justify-center gap-2 shadow-sm active:scale-95 uppercase tracking-tighter">
            <Save size={18} /> Save Record
          </button>
          <button onClick={handlePrint} className="flex-1 bg-indigo-600 text-white py-3 rounded-xl font-black text-sm hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95 uppercase tracking-tighter">
