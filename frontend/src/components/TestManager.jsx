...
import { Database, Plus, Trash2, Activity, Save, X, List, Layers } from 'lucide-react';
...
export default function TestManager() {
  ...
  const addParameterToMaster = async (testId) => {
    ...
    const referenceRange = prompt("Enter Reference Range (optional):") || "";

    try {
      const res = await axios.put(`${apiUrl}/api/tests/${testId}/parameters`, {
        paramName, unit, referenceRange, group: managedTest.name
      });
  ...
  const removeParameterFromMaster = async (testId, pIndex) => {
    if(!window.confirm("Remove this parameter from the master template?")) return;
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto h-auto lg:h-[calc(100vh-74px)] overflow-hidden flex flex-col font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <Layers className="text-indigo-600" /> Test Master Templates
          </h1>
          <p className="text-sm text-gray-500 font-medium italic">Configure what parameters appear in each diagnostic report</p>
        </div>
        <button 
          onClick={() => setShowAddForm(true)}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition-all shadow-lg active:scale-95"
        >
          <Plus size={18} /> New Test Template
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-100 flex items-center justify-center p-0 md:p-4 overflow-y-auto">
          <div className="bg-white md:rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-slate-200 min-h-screen md:min-h-0">
             ...
             <div className="bg-indigo-600 px-6 py-4 flex justify-between items-center text-white">
                <div>
                   <h2 className="font-black text-lg tracking-tight uppercase">{managedTest.name}</h2>
                   <p className="text-[10px] uppercase font-bold text-indigo-200 tracking-widest">{managedTest.category}</p>
                </div>
             ...
             <div className="p-6 h-[400px] overflow-y-auto custom-scrollbar">
                <div className="flex justify-between items-center mb-4">
                   ...
                   <button 
                    onClick={() => addParameterToMaster(managedTest._id)}
                   ...
                   {managedTest.parameters?.length === 0 ? (
                      ...
                      <div key={i} className="flex items-center justify-between p-3 bg-slate-50 border border-slate-200 rounded-xl group hover:border-indigo-300 transition-all">
                         <div className="flex-1">
                            <div className="text-xs font-black text-slate-800 uppercase">{p.paramName}</div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-indigo-50">
              <h2 className="font-bold text-indigo-900">Create New Master Test</h2>
      ...
      <div className="bg-white rounded-2xl shadow-xl border overflow-hidden flex-1 flex flex-col border-slate-200 min-h-[400px]">
        <div className="overflow-x-auto flex-1 custom-scrollbar">

          <table className="min-w-full divide-y divide-gray-100">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Master Test Name</th>
                <th className="px-6 py-4 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Department</th>
                <th className="px-6 py-4 text-center text-xs font-black text-slate-400 uppercase tracking-widest">Est. Price</th>
                <th className="px-6 py-4 text-center text-xs font-black text-slate-400 uppercase tracking-widest">Configured Params</th>
                <th className="px-6 py-4 text-right text-xs font-black text-slate-400 uppercase tracking-widest">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-50">
              {loading ? (
                 <tr><td colSpan="5" className="text-center py-20 font-medium text-gray-400 italic">Reading database...</td></tr>
              ) : tests.map((test) => (
                <tr key={test._id} className="hover:bg-indigo-50/50 transition-all group cursor-default">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-black text-slate-800 uppercase tracking-tight">{test.name}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="px-2 py-0.5 rounded bg-slate-100 text-[9px] font-black text-slate-500 uppercase tracking-tighter">
                      {test.category}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button 
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                    >
